...
下面是 利用黎曼积分法推导n为正整数的积分，其中分点坐标采用了算数平均数来确定
$$
\lim_{n \to \infty}\ \sum_{i=1}^{n} \left[\left(a+\frac{(b-a)\cdot i}{n}\right)^m \cdot \frac{(b-a)}{n} \right]
$$

下面是 利用黎曼积分法推导n为负整数的积分，其中分点坐标采用了几何平均数来确定

$$ \lim_{n \to \infty}\ \sum_{i = 0}^{n-1} \left[(a\cdot (\frac{b}{a})^{\frac{i+1}{n}}-a\cdot (\frac{b}{a})^{\frac{i}{n}}) \cdot \left[a\cdot (\frac{b}{a})^{\frac{i}{n}}\right]^{-m} \right] $$


$$
\lim_{n \to \infty} a^{1 - m} \left( \left( \frac{b}{a} \right)^{\frac{1}{n}} - 1 \right) \sum_{i=0}^{n-1} \left( \frac{a}{b} \right)^{\frac{i(m-1)}{n}}
$$

求和部分为有限等比数列，公比 $r = \left( \frac{a}{b} \right)^{\frac{m-1}{n}}$，利用等比数列求和公式：
若 m=1 则r=1
$$
\sum_{i=0}^{n-1} r^i = n
$$
将求和结果代入原式：
$$
\left( \left( \frac{b}{a} \right)^{\frac{1}{n}} - 1 \right) \cdot n
$$
求极限将 $\left(\frac{b}{a}\right)^{\frac{1}{n}}$ 表示为自然指数形式：  
   $$
   \left(\frac{b}{a}\right)^{\frac{1}{n}} = e^{\frac{\ln \left(\frac{b}{a}\right)}{n}}.
   $$
   因此原极限可改写为：  
   $$
   \lim_{n \to \infty} n \cdot \left( e^{\frac{\ln \left(\frac{b}{a}\right)}{n}} - 1 \right).
   $$
构造不等式（针对正项，这是切线不等式）对于足够小的正数 $x$（例如 $x = \frac{\ln \left(\frac{b}{a}\right)}{n}$ 且 $n \to \infty$），指数函数满足：  **后面的不等式需要继续考察，前面的属于常用切线不等式**
   $$
   1 + x \leq e^x \leq 1 + x + x^2. 
   $$  
   代入 $x = \frac{\ln \left(\frac{b}{a}\right)}{n}$ 得：  
   $$
   1 + \frac{\ln \left(\frac{b}{a}\right)}{n} \leq e^{\frac{\ln \left(\frac{b}{a}\right)}{n}} \leq 1 + \frac{\ln \left(\frac{b}{a}\right)}{n} + \left( \frac{\ln \left(\frac{b}{a}\right)}{n} \right)^2.
   $$  
   两边减 1 后：  
   $$
   \frac{\ln \left(\frac{b}{a}\right)}{n} \leq e^{\frac{\ln \left(\frac{b}{a}\right)}{n}} - 1 \leq \frac{\ln \left(\frac{b}{a}\right)}{n} + \frac{(\ln \left(\frac{b}{a}\right))^2}{n^2}.
   $$
   **应用夹逼定理**  将不等式两边乘以 $n$：  
   $$
   \ln \left(\frac{b}{a}\right) \leq n \cdot \left( e^{\frac{\ln \left(\frac{b}{a}\right)}{n}} - 1 \right) \leq \ln \left(\frac{b}{a}\right) + \frac{(\ln \left(\frac{b}{a}\right))^2}{n}.
   $$  
   当 $n \to \infty$ 时，右侧的 $\frac{(\ln \left(\frac{b}{a}\right))^2}{n} \to 0$，因此由夹逼定理得：  
   $$
   \lim_{n \to \infty} n \cdot \left( e^{\frac{\ln \left(\frac{b}{a}\right)}{n}} - 1 \right) = \ln \left(\frac{b}{a}\right) = \ln b -\ln a .
   $$

若 m>1 则r!=1
$$
\sum_{i=0}^{n-1} r^i = \frac{1 - r^n}{1 - r} = \frac{1 - \left( \frac{a}{b} \right)^{m-1}}{1 - \left( \frac{a}{b} \right)^{\frac{m-1}{n}}}
$$

将求和结果代入原式：
$$
a^{1 - m} \left( \left( \frac{b}{a} \right)^{\frac{1}{n}} - 1 \right) \cdot \frac{1 - \left( \frac{a}{b} \right)^{m-1}}{1 - \left( \frac{a}{b} \right)^{\frac{m-1}{n}}}
$$



当 $n \to \infty$时，我们分析表达式 $a^{1 - m} \left( \left( \frac{b}{a} \right)^{\frac{1}{n}} - 1 \right) \cdot \frac{1 - \left( \frac{a}{b} \right)^{m-1}}{1 - \left( \frac{a}{b} \right)^{\frac{m-1}{n}}}$的极限：

1. **等价无穷小替换**：
   - 当 $n \to \infty$ 时，$\left( \frac{b}{a} \right)^{\frac{1}{n}} - 1 \sim \frac{\ln(b/a)}{n}$。
   - 同理，$1 - \left( \frac{a}{b} \right)^{\frac{m-1}{n}} \sim \frac{(m-1)\ln(b/a)}{n}$。

2. **代入替换后的表达式**：
   $$
   a^{1 - m} \cdot \frac{\ln(b/a)}{n} \cdot \frac{1 - \left( \frac{a}{b} \right)^{m-1}}{\frac{(m-1)\ln(b/a)}{n}}
$$

3. **化简**：
   - 分子和分母中的 $\frac{\ln(b/a)}{n}$项约去，得到：
     $$
     a^{1 - m} \cdot \frac{1 - \left( \frac{a}{b} \right)^{m-1}}{m - 1}
     $$
   - 将 $\left( \frac{a}{b} \right)^{m-1}$ 转换为$\frac{a^{m-1}}{b^{m-1}}$，进一步化简：
     $$
     a^{1 - m} \cdot \frac{b^{m-1} - a^{m-1}}{b^{m-1}(m - 1)} = \frac{a^{1 - m}(b^{m-1} - a^{m-1})}{b^{m-1}(m - 1)}
     $$
   - 整理为：
     $$
     \frac{1}{m - 1} \left( \frac{1}{a^{m - 1}} - \frac{1}{b^{m - 1}} \right)
     $$

**最终极限结果**：
$$
\boxed{\frac{1}{m - 1} \left( \frac{1}{a^{m - 1}} - \frac{1}{b^{m - 1}} \right)}
$$