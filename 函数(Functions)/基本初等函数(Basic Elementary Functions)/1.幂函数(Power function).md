...
$$
\lim_{n \to \infty}\ \sum_{i=1}^{n} \left[\left(a+\frac{(b-a)\cdot i}{n}\right)^n \cdot \frac{(b-a)}{n} \right]
$$
...
求极限将 $\left(\frac{b}{a}\right)^{\frac{1}{n}}$ 表示为自然指数形式：  
   ...
   $$
构造不等式（针对正项，这是切线不等式）对于足够小的正数 $x$（例如 $x = \frac{\ln \left(\frac{b}{a}\right)}{n}$ 且 $n \to \infty$），指数函数满足：  
   $$
